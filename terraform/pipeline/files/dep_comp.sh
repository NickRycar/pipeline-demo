...
dep_count=$(curl -s https://bldr.habitat.sh/v1/depot/channels/nrycar/stable/pkgs/national-parks/latest\?target\=x86_64-linux | jq -r '.tdeps[].name' | wc -l)
curr_count=0
...
do
  stable_name=$(curl -s https://bldr.habitat.sh/v1/depot/channels/nrycar/stable/pkgs/national-parks/latest\?target\=x86_64-linux | jq -r ".tdeps[$curr_count].name")
  stable_ver=$(curl -s https://bldr.habitat.sh/v1/depot/channels/nrycar/stable/pkgs/national-parks/latest\?target\=x86_64-linux | jq -r ".tdeps[$curr_count].version")
  stable_release=$(curl -s https://bldr.habitat.sh/v1/depot/channels/nrycar/stable/pkgs/national-parks/latest\?target\=x86_64-linux | jq -r ".tdeps[$curr_count].release")
